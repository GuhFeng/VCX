...
class Edge:
    ...
    def __hash__(self):
        v1 = self.v1
        v2 = self.v2
        if v1[0] < v2[0] or (v1[0] == v2[0] and v1[1] < v2[1]) or (
    ...
    def get_str(self, v1, v2):
        return hash(
            str(v1[0]) + str(v1[1]) + str(v1[2]) + str(v2[0]) + str(v2[1]) +
            str(v2[2]))
...
class BPA:
    ...
    def find_rad(self):
        self.radius = np.average(
            self.pc.compute_nearest_neighbor_distance()) / 1.5 * 2

    def construct_voxel(self):
        box_pts = np.asarray(
            self.pc.get_axis_aligned_bounding_box().get_box_points())
        ...
        self.voxel = [[[[] for i in range(self.z_width)]
                       for j in range(self.y_width)]
                      for k in range(self.x_width)]

        r = self.radius

        for pt, n in zip(np.asarray(self.pc.points),
                         np.asarray(self.pc.normals)):
    ...
    def get_neighbors(self, pt, rad):
        ...
        for i in range(max(0, x - voxel_dist),
                       min(x + voxel_dist + 1, self.x_width)):
            for j in range(max(0, y - voxel_dist),
                           min(y + voxel_dist + 1, self.y_width)):
                for k in range(max(0, z - voxel_dist),
                               min(z + voxel_dist + 1, self.z_width)):
    ...
    def get_ball_centers(self, p1, p2, p3, r):
        ...
        numerator_left = np.cross(
            np.linalg.norm(p3 - p1)**2 * (np.cross(p2 - p1, p3 - p1)),
            (p2 - p1))
        numerator_right = np.cross(
            np.linalg.norm(p2 - p1)**2 * (np.cross(p3 - p1, p2 - p1)),
            (p3 - p1))
    ...
    def find_seed_triangle(self):
		...
		"""
        for pt, norm in zip(np.asarray(
                self.pc.points), np.asarray(
                    self.pc.normals)):  #TODO: change to pts not yet used
            ...
            for i in range(len(neighbors) - 1):
                for j in range(i + 1, len(neighbors)):
                    ...
                    center1, center2, unit_vect = self.get_ball_centers(
                        pt, pt_1, pt_2, self.radius)
                    triangle = None
                    if len(center1):
                        if self.validate_ball_center(center1, unit_vect,
                                                     [pt, norm]):
                            triangle = Triangle(pt, pt_1, pt_2)
                        elif self.validate_ball_center(center2, -unit_vect,
                                                       [pt, norm]):
                            triangle = Triangle(pt, pt_1, pt_2)
                        if triangle:
                            v1 = Vertex(pt)
