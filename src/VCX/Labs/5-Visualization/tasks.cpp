...
#define CAST_FLOAT(a, i) (*(((float *) &a) + i))
    ...
    bool PaintParallelCoordinates(Common::ImageRGB & input, InteractProxy const & proxy, std::vector<Car> const & data, bool force) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            PrintText(input, glm::vec4(0, 0, 0, 1), glm::vec2(0.08 + i * 0.14, 0.92), 0.02, datarange_str[1][i]);
            PrintText(input, glm::vec4(0, 0, 0, 1), glm::vec2(0.08 + i * 0.14, 0.08), 0.02, datarange_str[0][i]);
        }
        for (int i = 0; i < data.size(); i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                DrawLine(input, glm::vec4(0.8 * i / data.size(), 0.1, 0.8 - 1.0 * i / data.size(), 0.2), glm::vec2(0.08 + j * 0.14, y1), glm::vec2(0.22 + j * 0.14, y2), 2);
            }
        }
